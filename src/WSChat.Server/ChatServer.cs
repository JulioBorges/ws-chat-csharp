    ...
    class ChatServer : TCPServer
    {
        ...
        private void VerifyData()
        {
            while (this.Running)
            {
                try
                {
                    lock (threadLock)
                    {
                        if (SessionManager.SessionList.Any())
                        {
            ...
            if (session.User != null)
            {
                ...
                switch (message.Type)
                {
                    ...
                    case MessageType.JoinRoom:
                        ...
                        try
                        {
                            if (!RoomManager.RoomExists(msgData[0]))
                            {
                                RoomManager.AddRoom(new Room(msgData[0]));
                                HandleLog("" + session.User.NickName + " : chatroom has been created: " + msgData[0]);
                            }

                            if (RoomManager.RoomsList.Any(x => x.Name == msgData[0]))
                            {
                    ...
                    case MessageType.ListUsers:
                        ...
                        var users = SessionManager.SessionList.Where(s => s.User != null &&
        ...
        private void CheckIsRunning()
        {
            while (this.Running)
            {
                try
                {
                    foreach (var session in SessionManager.SessionList)
                    {
        ...
        private void BroadcastToRoom(Session session, string message)
        {
            ...
            if (room != null && message != "")
            {
                ...
                var sessionsFromRoom = SessionManager.SessionList
